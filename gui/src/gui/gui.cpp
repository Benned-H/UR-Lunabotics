...
GUI::GUI( const double& diameterArg ) : ROBOT_DIAMETER( diameterArg ), hasPath( false ), hasLookahead( false ){}
...
// TODO - We should only need to draw the static obstacles once.
...
void GUI::update() {
	drawPose( pose, 0.0, 1.0, 0.0 );
	if (hasPath) drawPath( path, 0.847, 0.204, 0.922 );
	if (hasLookahead) drawLookahead( lookahead, 0.1, 0.0, 0.8, 1.0 );
	if (obstacles.data.size() > 0) drawObstacles( 1.0, 0.1, 0.1 );
}
...
void GUI::drawObstacles( double r, double g, double b ){
	for (int i = 0; i < obstacles.data.size(); i++) {
