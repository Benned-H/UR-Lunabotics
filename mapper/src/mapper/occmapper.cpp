...
*/
OccMapper::OccMapper( const double& resolutionArg, const unsigned int& mapWidthArg, const unsigned int& mapHeightArg,
                      const geometry_msgs::Pose& origin, const double& obstacleWidthArg, const int& thresholdArg,
                      const double& p0, const double& p_free, const double& p_occ ) : RESOLUTION( resolutionArg ){
  // TODO - Correctly initialize all class member variables

  if ( RESOLUTION == 0.0 )
    RESOLUTION = 0.01;
	map.info.resolution = resolutionArg;
	map.info.width = mapWidthArg;
	map.info.height = mapHeightArg;
	...
	occ_steps = std::floor( obstacleWidthArg / RESOLUTION );
	...
	for( int i = 0; i < scan.ranges.size(); i++ ){
      ...
      int cell_index = point_to_index(x, y);
		...
		for( std::set<int>::iterator free = free_cells.begin(); free != free_cells.end(); ++free ){
			double result = map.data[*free] + l_free - l0;
			if( result < 0.0 ){
				result = 0;
			} else if( result > 100.0 ){
				result = 100;
			}
      map.data[*free] = (int) result;
		}
		
		for( std::set<int>::iterator occ = occupied_cells.begin(); occ != occupied_cells.end(); ++occ ){
			double result = map.data[*occ] + l_occ - l0;
			if( result < 0.0 ){
				result = 0;
			} else if( result > 100.0 ){
				result = 100;
			}
			map.data[*occ] = (int) result;
		}
	}
